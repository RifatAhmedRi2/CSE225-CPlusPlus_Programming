...
//Modified Breadth First Search to Find the Length of the Shortest Path
template<class VertexType>
void GraphType<VertexType>::ModBreadthFirstSearch(VertexType startVertex, VertexType endVertex)
{
    QueType<VertexType> queue;
    QueType<VertexType> vertexQ;
    bool found = false;
    ...
    while(!stack.IsEmpty())
    {
        ...
        stack.Pop();
        vertex2 = stack.Top();
        if(FoundEdge(vertex1,vertex2) && !stack.IsEmpty()){
            counting++;
            stack.Pop();
        }
    }
    cout << endl;
    cout<<"Length of the Shortest path between the two given vertices: "<<counting++<<endl;
    if (!found)
        cout << "Path not found." << endl;
}

/*
//Modified Breadth First Search to Find the Length of the BFS
template<class VertexType>
void GraphType<VertexType>::ModBreadthFirstSearch(VertexType startVertex, VertexType endVertex)
{
    QueType<VertexType> queue;
...
//Function to Determine the OutDegree of a given Vertex
template<class VertexType>
...
template<class VertexType>
char* GraphType<VertexType>::FoundEdge(VertexType fromVertex, VertexType toVertex)
{
    int row = IndexIs(vertices, fromVertex);
    int col = IndexIs(vertices, toVertex);
    if(edges[row][col] == 1)
        return "There is an Edge";
    else
